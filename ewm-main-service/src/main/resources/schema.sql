...
CREATE TYPE IF NOT EXISTS comment_status_enum AS ENUM ('CREATED', 'CONFIRMED', 'REJECTED', 'DELETED');

CREATE TABLE IF NOT EXISTS comments(
    comment_id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    comment_text VARCHAR(7000) NOT NULL,
    comment_created_date TIMESTAMP NOT NULL,
    comment_status comment_status_enum NOT NULL,
    comment_user_id BIGINT NOT NULL,
    comment_event_id BIGINT NOT NULL,
    CONSTRAINT PK_COMMENTS PRIMARY KEY (comment_id),
    CONSTRAINT FK_COMMENTS_USER FOREIGN KEY (comment_user_id)
        REFERENCES users(user_id)
        ON DELETE CASCADE,
    CONSTRAINT FK_COMMENTS_EVENT FOREIGN KEY (comment_event_id)
        REFERENCES events(event_id)
        ON DELETE CASCADE
);